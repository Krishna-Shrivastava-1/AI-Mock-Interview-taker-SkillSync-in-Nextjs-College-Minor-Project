...
export default function ProfileLayout({ children, params }) {
  ...
  const [userDatafromparam, setuserDatafromparam] = useState([])
    ...
    const pathlastname = pathname.split('/')
    // console.log(pathlastname[pathlastname.length-1])
...
<div className={`${pathlastname[pathlastname.length-1] === id && 'border-b-[2px]  border-sky-500'} p-2 cursor-pointer  select-none hover:bg-neutral-900`}>
